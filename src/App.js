        ...
        return (
            <form id='loginForm' onSubmit={handleLogin}>
                <div>
                    ...
                    <input id='usernameInput' type="text" value={username} placeholder="Enter Username" name="uname" onChange={({ target }) => setUsername(target.value)} required/>
                    <label style={{ display: 'block' }}><b>Password</b></label>
                    <input id='passwordInput' type="password" value={password} placeholder="Enter Password" name="psw" onChange={({ target }) => setPassword(target.value)} required/>
                    ...
                    blogs.map((blog, idx) =>
                        <div key={Math.random()} style={{ outline: '1px dashed black', margin: '4px' }}>
                            ...
                            <button onClick={() => {
                                ...
                                let newBlogs = [...blogs]
                                newBlogs = newBlogs.filter(x => x.id !== blog.id)
                                setBlogs(newBlogs)
                            }}>delete</button>
                        </div>
                    )
